...
class RQ_Prompt(nn.Module):
    def __init__(self, length, embed_dim, embedding_key, pool_size, top_k, prompt_init='uniform', prompt_pool=True,
        ...
        residual_key_shape = (pool_size, embed_dim)
    ...
    def forward(self, x_embed, iseval, task_id):
        ...
        reduce_sim =  self.reduce_similarity(x_embed, prompt_norm, idx)
        ...
        _, resdiual_idx = torch.topk(resdiual_similarity, k=self.top_k, dim=1)  # B, top_k
        ...
        out['reduce_sim'] = reduce_sim + resdiual_reduce_sim
