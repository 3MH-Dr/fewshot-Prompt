...
class Expert(nn.Module):
    ...
    def forward(self, task, task_id, iseval=False, curr_rel=''):
        ...
        return p_score, n_score, rel_q.detach().clone()
    
class MoE_metaR(nn.Module):
    def __init__(self, dataset, parameter):
        super(MoE_metaR, self).__init__()
        ...
        if parameter['dataset'] == 'Wiki-One':
            self.relation_learner = RelationMetaLearner(parameter, parameter['few'], embed_size=50, num_hidden1=250,
                                                        num_hidden2=100, out_size=50, dropout_p=self.dropout_p)
            self.routers = nn.ModuleList([Router(50 * 2, i) for i in range(parameter['num_tasks'] - 1) if i > 0])

        elif parameter['dataset'] == 'NELL-One':
            self.relation_learner = RelationMetaLearner(parameter, parameter['few'], embed_size=100, num_hidden1=500,
                                                        num_hidden2=200, out_size=100, dropout_p=self.dropout_p)
            self.routers = nn.ModuleList([Router(100 * 2, i) for i in range(parameter['num_tasks']) if i > 1])
    ...
    def forward(self, task, task_id, iseval=False, curr_rel=''):
        ...
        # use experts on support when eval?
        if task_id > 1:
            rels = []
            for i in range(task_id - 1):  # 2: 1 expert, task_id: 1 
                _, _, rel = self.experts[i](task, i + 1, iseval, curr_rel=curr_rel)
                rels.append(rel)
            rels.append(rel_q)
            rel_q = torch.stack(rels, dim=-1)
            probs = self.routers[task_id - 2](support)
            rel_q = (rel_q * probs.reshape(probs.shape[0], 1, 1, 1, probs.shape[1])).sum(dim=-1)
            
            # Debug
            with torch.no_grad():
                for rel in rels:
                    print(rel.shape)
                    # sys.stdout.write(f'{F.cosine_similarity(rel, rel_tmp, dim=-1).sum()}')
                    sys.stdout.flush()
                print('-----')
                # sys.stdout.write(f'{probs}')
