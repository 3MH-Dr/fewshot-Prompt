...
class RelationMetaLearner(nn.Module):
    def __init__(self, parameter, few, embed_size=100, num_hidden1=500, num_hidden2=200, out_size=100, dropout_p=0.5):
        ...
        self.rel_fc1 = nn.Sequential(OrderedDict([
            ...
            # ('bn', nn.BatchNorm1d(few, affine=False, track_running_stats=False)),
            ('relu', nn.LeakyReLU()),
...
class Lora_metaR(nn.Module):
    def __init__(self, dataset, parameter):
        super(Lora_metaR, self).__init__()
        ...
        # add and init lora
        add_lora(self)
        self.apply(apply_to_lora(lambda x: torch.nn.init.ones_(x.lora_B)))
    ...
    def forward(self, task, task_id, iseval=False, curr_rel=''):
        ...
        rel = self.relation_learner(support, task_id, iseval)
        rel.retain_grad()
